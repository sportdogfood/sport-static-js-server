...
checkForIdentification() {
    ...
    // Check for identification using cookies 
    if (cookies['fx_customer_sso'] || cookies['fx_customerId'] || cookies['fx_customer_jwt'] || cookies['fx_customer'] || cookies['fx_customer_em'] || cookies['sporturl'] || window.fx_customerEmail || window.fx_customerId) {
        ...
        ssoToken = cookies['fx_customer_sso']; // Get SSO token if available
    }
...
findCustomer() {
    ...
    if (!customerId) {
        ...
        return; // Return early if customerId is not defined
    }

    console.log(`Attempting to find customer with ID: ${customerId}`);

    // Load the fxcustomer.js script if it's not already loaded
...
// Function to initiate customer fetching
initiateCustomerFetch(customerId) {
    if (typeof fxCustomerInit === 'function') {
        fxCustomerInit(customerId)
            .then(response => {
                if (response && response.ok) {
                    console.log("Customer found, updating session with customer data.");
            ...
            .catch(error => {
                console.error("Error in fetching customer data from fxcustomer.js:", error);
                // Continue with other checks if needed
                this.continueIdentificationChecks();
            });
    } else {
        console.error("fxCustomerInit function is not defined in fxcustomer.js.");
...
continueIdentificationChecks() {
    console.log("Continuing with additional checks as fxcustomer.js failed.");
    // Placeholder 
},
...
// Define buttonMaster to handle the state of buttons and status-div
function defineButtonMaster() {
    window.buttonMaster = function (status, caller) {
        ...
        // Log the current existence state of buttons and div
        console.log(`[buttonMaster] loginButton found: ${!!loginButton}, logoutButton found: ${!!logoutButton}, statusDiv found: ${!!statusDiv}`);
        ...
        if (status === 'logged in') {
            if (loginButton) {
                ...
                console.log('[buttonMaster] loginButton set to hidden');
            } else {
                console.error('[buttonMaster] loginButton is not found in the DOM.');
            }

            if (logoutButton) {
                logoutButton.style.display = 'inline';
                console.log('[buttonMaster] logoutButton set to visible');
            } else {
                console.error('[buttonMaster] logoutButton is not found in the DOM.');
            }
        } else if (status === 'logged out') {
            if (loginButton) {
                loginButton.style.display = 'inline';
                console.log('[buttonMaster] loginButton set to visible');
            } else {
                console.error('[buttonMaster] loginButton is not found in the DOM.');
            }
            ...
            if (logoutButton) {
                ...
                console.log('[buttonMaster] logoutButton set to hidden');
            } else {
                console.error('[buttonMaster] logoutButton is not found in the DOM.');
            }
        } else {
...
// Add login, logout, and auth button click handlers programmatically
document.addEventListener('DOMContentLoaded', () => {
    attachButtonEventListeners();

    // Initial call to buttonMaster to set the correct state based on the session
    ...
    scriptsToLoad.forEach(scriptInfo => {
        if (!document.getElementById(scriptInfo.id)) {
            ...
            if (typeof window[scriptInfo.initFunction] === 'function') {
                ...
                console.warn(`${scriptInfo.initFunction} function not found. `);
            }
        }
    });
});
