// ui.js
import { sessionAuth, sessionGeo, sessionEnd } from './sessionManager.js';
...
export function setupUIEventListeners() {
    ...
    if (logoutButton && !logoutButton.listenerAdded) {
        logoutButton.addEventListener('click', handleLogoutHandler);
    ...
    if (refreshGeoButton) {
        refreshGeoButton.addEventListener('click', () => {
            sessionGeo.geoFunctions.refreshGeoLocationData();
            console.log('Refresh Geo button clicked.');
        });
    ...
    if (refreshCustomerZoomButton) {
        refreshCustomerZoomButton.addEventListener('click', () => {
            sessionGeo.refreshCustomerZoom();
            console.log('Refresh CustomerZoom button clicked.');
        });
...
function authenticateCustomerHandler(event) {
    ...
    if (sessionAuth.authFunctions && typeof sessionAuth.authFunctions.authenticateCustomer === 'function') {
        sessionAuth.authFunctions.authenticateCustomer(email, password);
    } else {
        console.error('authenticateCustomer function is not available.');
    }
}
 ...
 * @param {Event} event - Click event
 */
function handleLogoutHandler(event) {
    event.preventDefault();
    if (typeof sessionEnd.handleLogout === 'function') {
        sessionEnd.handleLogout();
    } else {
